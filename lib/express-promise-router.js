        ...
        Promise.resolve(ret).then(
            function(d) {
                if (d === 'next') {
                    ...
                    next(
                        new Error(
                            'Your responses should never resolve to a string, as you may ' +
                                'end up sending "route" or "next" by accident. Please wrap your json in an ' +
                                'object.'
                        )
                    );
                } else if (d !== undefined && res) {
                    // content-type
                    ...
                    if (req.get('HTTP_X_REQUESTED_WITH')) {
                        ...
                        res.end(JSON.stringify(d || null, null, 2));
                    }
                }
            },
            function(err) {
